...
0. **Context seeding (before authoring):** if the new skill shares a domain with an existing proven skill, call `tfc_context_get(name=<similar_skill>, task=<new_skill_intent>)` to pull ranked grounded prose. Seed SKILL.md from it. Context sourced this way carries a per-section `source:` provenance line -- it is grounded, not fabricated.
   For an existing skill being enhanced (not rebuilt): read the current SKILL.md first. Enhancing a proven skill routes to `build` (not `forge`) unless the enhancement adds a genuinely new recurring class of capability.
...
| `--context <skill>` | overlay | before Stage 4, seeds domain knowledge from an existing proven skill via tfc_context_get; skip when no prior skill in the domain |
...
| Context Seeding | `tfc_context_get` (retrieve ranked grounded body for a task -- model-free, deterministic; use at Stage 4 before authoring) , `tfc_context_fill` (offline fill prompt -- Claude executes out-of-band; NO_SOURCES beats empty-but-wrong) , `tfc_context_discover` (find all domains from disk + manifests; zero .ts edits to add a domain) , `tfc_context_coverage` (angle-completeness: answered/declared -- one filled file out of twelve is not done) |
| Govern | `tfc_graph` , `tfc_recommend` , `tfc_compose` , `tfc_context` / `tfc_context_audit` / `tfc_context_update` / `tfc_context_coverage` / `tfc_context_discover` , `tfc_capture` , `tfc_pack_bridge` , `tfc_decay` , `tfc_replay` , `tfc_portfolio` , `tfc_list` , `tfc_doctor` , `tfc_relink` |
...
- **voice-gate-managed-block-false-positive:** tfc_validate's voice/path gates fire on the managed TFC:PREAMBLE-HOOK block and `${TFC_HOME:-$HOME/.future-code}` portable fallback. These are regenerated by tfc_install -- the author cannot fix them and should not (removing $HOME breaks portability). They dock rubric points honestly; do not patch the managed block to silence them.
- **enhance-routes-to-build-not-forge:** a "make this skill better" request routes to `build` (enhance-in-place), not `forge`. A multi-axis improvement ask ("better + faster + more capable") often collapses to ONE mechanism change; forging a duplicate when the skill already exists bloats the portfolio. Read the current SKILL.md before staging the route.
- **context-fill-no-sources:** tfc_context_fill returns NO_SOURCES when a skill has no grounded inputs (empty SKILL.md, no spec, no prior eval). This is correct and honest. Do not fabricate context to bypass it; author grounded SKILL.md sections first, then re-run fill.
- **reachable-vs-proven:** tfc_lane returns reachable:false until tfc_install creates the managed CLAUDE_SKILLS symlink, even when the eval score is 1.0. A skill can be eval_proven but unreachable. Always run tfc_install before reporting a skill as live.
